...
BOOL CALLBACK OnVisitorList(HELE hEle, HELE hEventEle, int nIndex) 
{
	...
	//HWINDOW hWindow=XModalWnd_CreateWindow(410, 120, L"登记来访信息？", XWnd_GetHWnd(g_hWindow));
	if(hWindow)
	{
	...
	if ( ret )
	{
		...
		{	
            ...
            if(g_bQRcode)
			{
				...
				VISITORINFO visitor;
				memset((char *)&visitor, 0, sizeof(VISITORINFO));
				GetAESJson(g_szComBarcode,json);
				getYueJSon(json,visitor);
 				wchar_t wBuf[256 + 1] = {0};
				memset((char *)wBuf, 0, sizeof(wBuf));
				MultiByteToWideChar(CP_ACP, 0, visitor.szPhone, -1, wBuf, 256);
...
//访客签离
BOOL VisitorLogOut(char *pszBarCode)
{
...
DWORD WINAPI ReadBarCodeThread2(LPVOID lpVoid)
{
	CBarCode tBarCode;
	int iBarCom;
	char szT[10000] = {'\0'},szBarPort[4]={'\0'};
	BOOL bRet = false;
...
BOOL getYueJSon(char *js,VISITORINFO &visitor)
{
    ...
    else
    {
		...
		strcpy(g_szQRvisitime,json_value->valuestring);
		
		
        //来访人数
		/*json_value = cJSON_GetObjectItem( json , "pn");
		if (strcmp(json_value->valuestring , "")!=0)
		{
...
/*BOOL CALLBACK OnSaveQRBtnClick(HELE hELE, HELE hEventEle)
{
	return false;
}

BOOL CALLBACK OnCancleBtnClick(HELE hELE, HELE hEventEle)
{
	InitData();
	return false;
}*/
